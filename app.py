...
<style>
    ...
    * { font-family: 'Pretendard', sans-serif !important; }
    ...
    /* 1. 화면 전체 스크롤 잠금 해제 (Sticky 작동 필수 조건) */
    ...
    /* 3. 좌우 기둥이 키 맞추기(Stretch) 금지 -> 빈 공간 확보 */
    [data-testid="stHorizontalBlock"] {
        align-items: flex-start !important;
    }

    /* 4. 오른쪽 그래프 기둥을 화면 상단에 고정 */
    /* div[data-testid="column"]:has(#sticky-anchor) : 닻이 있는 기둥을 찾음 */
    div[data-testid="column"]:has(#sticky-anchor) {
        position: -webkit-sticky !important;
        position: sticky !important;
        top: 80px !important; /* 상단 메뉴바 아래에 위치 */
        z-index: 1000 !important;
        height: fit-content !important; /* 내용물 크기만큼만 높이 잡음 */
    ...
    with st.spinner("🕵️‍♂️ 1타 강사가 문제를 분석하고 있습니다... 잠시만 기다려주세요."):
        try:
            ...
            **[Python 코드 작성 시 절대 주의사항]**
            1. **Numpy 오류 방지**: `if array:` 처럼 배열을 직접 조건문에 쓰지 마. 반드시 `if array.size > 0:` 또는 `if np.any(array):` 를 사용해.
            2. **비율 고정**: `ax.set_aspect('equal')` 필수.
            3. **크기**: `plt.figure(figsize=(6, 6))`
            4. **식 표시**: 그래프 식은 범례(Legend) 대신 **그래프 선 근처에 텍스트**로 표시. (겹치지 않게 Offset 사용)
            5. **글씨 크기**: `fontsize=9` 통일.
            6. **언어**: 한글 깨짐 방지를 위해 **모두 영어(English)**로 작성.
...
if st.session_state.analysis_result:
    ...
    with col_graph:
        # [핵심] Sticky Anchor 심기 (이게 있어야 따라옵니다)
